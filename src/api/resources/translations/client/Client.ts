...
export class Translations {
    ...
    public async postQuestion(
        request: Squidex.AskDto,
        requestOptions?: Translations.RequestOptions
    ): Promise<string[]> {
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: urlJoin(
                (await core.Supplier.get(this._options.environment)) ?? environments.SquidexEnvironment.Default,
                `api/apps/${this._options.appName}/ask`
            ),
            method: "POST",
            headers: {
                Authorization: await this._getAuthorizationHeader(),
            ...
            contentType: "application/json",
            body: await serializers.AskDto.jsonOrThrow(request, { unrecognizedObjectKeys: "strip" }),
            timeoutMs: requestOptions?.timeoutInSeconds != null ? requestOptions.timeoutInSeconds * 1000 : 60000,
        });
        if (_response.ok) {
            return await serializers.translations.postQuestion.Response.parseOrThrow(_response.body, {
                unrecognizedObjectKeys: "passthrough",
                allowUnrecognizedUnionMembers: true,
                allowUnrecognizedEnumValues: true,
                breadcrumbsPrefix: ["response"],
